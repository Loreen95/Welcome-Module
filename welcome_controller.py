...
class welcomeController:
    ...
    def welcome_command(self, request):   
        welcome = self.get_welcome_window()
        self.bot.send_private_message(request.sender.char_id, ChatBlob("Welcome", welcome))

    def get_welcome_window(self, request, char_id):
        nsql = "SELECT n.*, p.name AS author FROM news n LEFT JOIN player p ON n.char_id = p.char_id WHERE n.deleted_at = 0 AND n.sticky = 0 ORDER BY n.created_at DESC LIMIT 1"
        news = self.db.query(nsql)
        if news:
        ...
        for timer in data: 
            tara = self.timer_controller.get_timer("Tarasque")
            blob += "\n<white>Tarasque will be <end><green>killable<end><white> in: %s" % (self.util.time_to_readable(timer.created_at + timer.duration - t, max_levels=None))
            gaunt = self.timer_controller.get_timer("Gauntlet")
            blob += "\n<white>Vizaresh will be <end><green>killable<end><white> in: %s" % (self.util.time_to_readable(timer.created_at + timer.duration - t, max_levels=None))
            gaubuff = self.timer_controller.get_timer("Gaubuff")
            blob += "\n<white>Gaubuff will be <end><green>available<end><white> for: %s" % (self.util.time_to_readable(timer.created_at + timer.duration - t, max_levels=None))
            if self.timer_controller.get_timer("Gaubuff") == None:
                blob += "\n<white>Gaubuff is <end><red>unavailable.<end>"        
                blob += "\n\n"

        blob += "\n<header>::Personal Info & Preferences::<end>\n"
        
        main_id = self.alts_service.get_main(request.sender.char_id)
        ...
        data = ("SELECT COALESCE(p.name, m.char_id) AS name, m.char_id, m.auto_invite FROM members m LEFT JOIN player p ON m.char_id = p.char_id ORDER BY p.name ASC")
        pref = self.db.query(data)
        blob += "\n<white>Your autoinvite is currently:<end> %s [%s|%s]" % ("<green>On<end>" if pref == 1 else "<red>Off<end>", autoinv_link, autoin_link)
    ...
    def priv_logon_event(self, event_type, event_data):     
        welcome = self.get_welcome_window(False, char_id)
        self.bot.send_private_message(char_id, ChatBlob("Welcome", welcome))
